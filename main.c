...
void write_to_file(char *file_name, students **arr, int size)
{
    ...
    // Записываем отсортированные имена студентов
    for (int i = 0; i < size; i++)
    {
        write(fd, arr[i]->name, strlen(arr[i]->name));
        write(fd, "\n", 1);
    }
...
int main()
{
    ...
    if (students_array != NULL)
    {
        ...
        write_to_file(file_name, students_array, array_size);
